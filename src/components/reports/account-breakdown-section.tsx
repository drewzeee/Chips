    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="border border-gray-700 rounded-lg p-4 bg-gray-800/50 backdrop-blur-sm">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-sm font-semibold text-gray-200">Filter Accounts</h4>
              <button
                onClick={toggleAllAccounts}
                className="text-xs text-blue-400 hover:text-blue-300 font-medium px-3 py-1 rounded-md bg-gray-700/50 hover:bg-gray-700 transition-colors"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
              {accountNames.map((accountName) => (
                <label
                  key={accountName}
                  className="flex items-center space-x-3 text-sm cursor-pointer hover:bg-gray-700/30 p-3 rounded-md transition-colors border border-gray-700/50 hover:border-gray-600"
                >
                  <input
                    type="checkbox"
                    checked={visibleAccounts.has(accountName)}
                    onChange={() => toggleAccountVisibility(accountName)}
                    className="rounded border-gray-600 bg-gray-700 text-blue-500 focus:ring-blue-500 focus:ring-offset-gray-800 w-4 h-4"
                  />
                  <span className="text-gray-100 font-medium flex-1">{accountName}</span>
                </label>
              ))}
            </div>
            {filteredAccountNames.length === 0 && (
              <p className="text-sm text-gray-400 mt-3 italic">Select at least one account to display data.</p>
            )}
          </div>
          ...
          {filteredAccountNames.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg">
              <div className="text-gray-500">Select accounts to view chart</div>
            </div>
          )}
        </div>
      </CardContent>
